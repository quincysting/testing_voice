...
resource "aws_ecs_task_definition" "imap_server" {
  ...
  # depends_on = [
  #   aws_lb_listener.imap_server,
  #   aws_ecs_task_definition.imap_server,
...
resource "aws_ecs_service" "imap_server" {
  ...
  depends_on = [aws_lb_listener.imap_server]
...
resource "aws_ecs_cluster_capacity_providers" "example" {
  ...
  #capacity_providers = ["kamailio-lab-ec2-capacity-provider", "voicemail-lab-ec2-capacity-provider", aws_ecs_capacity_provider.ec2.name]
  ...
  depends_on = [aws_ecs_service.imap_server]
}


resource "aws_autoscaling_group" "ecs" {
  name                = "${var.app_name}-ecs-asg"
  vpc_zone_identifier = var.imap_subnets
...
resource "aws_security_group" "alb" {
  ...
  dynamic "ingress" {
    for_each = toset([80, 8080, 443])
    content {
      protocol    = "tcp"
      from_port   = ingress.value
      to_port     = ingress.value
      cidr_blocks = ["0.0.0.0/0"] # to be updated after receiving subnets information
      description = "Allow TCP ${ingress.value} from all sources"
    }
  }
